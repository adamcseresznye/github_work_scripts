...
#Folder location:
all_files = glob.glob('/Users/adamcseresznye/Desktop/ 211218_e-waste-FI-LV_B2/**/a-all.txt', 
                   recursive = True)
#Read in datafiles:
li = []
...
for filename in all_files:
    df = pd.read_fwf(filename, colspecs= colspecs, skiprows = 19, skipfooter = 5,
                     names = ['number', 'compound', 'Rt',
                              'Qion', 'Response', 'Concentration']) 
    li.append(df)
...
#read in file names:
names = [os.path.dirname(x) for x in glob.glob('/Users/adamcseresznye/Desktop/ 211218_e-waste-FI-LV_B2/**/a-all.txt',recursive = True)]
...
df = pd.DataFrame({'Reponse_ID': {0: 'mass_CB 143 (IS)', 1: 'mass_13C HCB', 2: 'mass_CB 207 (RS)'},
...
for i in RF_df:
    temp = pd.DataFrame(
        {
            'CB143(IS)_RRF': [(RF_df[i][0] * RF_df[i][5]) / (RF_df[i][3] * RF_df[i][2])],
...
for i in no_IS:
    temp = pd.DataFrame(
        {
           'CB143(IS)_extracted_pg': [(no_IS.loc['CB 143 (IS)'][i] * 2500 ) / (no_IS.loc['CB 207 (RS)'][i] * AVG_RRF.loc['CB143(IS)_RRF'])],
           '13CHCB_extracted_pg': [(no_IS.loc['13C HCB'][i] * 2500 ) / (no_IS.loc['CB 207 (RS)'][i] * AVG_RRF.loc['13CHCB_RRF'])]
...
Recovery = pd.DataFrame(list(zip(CB143_recovery, HCB_recovery)),
...
blank_filtered = concentration.filter(regex = 'blank') \
        .iloc[4:,:]
...
#in the QC
theoretical_assigned = {'CB 101': 1.44, 
     'CB 99': 0.508, 
     'CB 118': 0.331,
     'CB 153' : 0.759,
     'CB 138' : 0.865,
...
sample_concentration_pg = concentration.drop(list(concentration.filter(regex = '(IS-RS|blank|AMAP)')), 
                                             axis = 1) \
...
sample_list_filled_in = pd.read_csv('/Users/adamcseresznye/Desktop/Python/Pandas_for_everyone/sample_list_empty.csv', index_col = 0)\
    .squeeze()
